...
int v[10010];
ull word[10010],x,dic[2000010],p=107;
char s[10000010];
FILE *stream;
...
int main(int argc,char** argv)
{
	...
	for(i=3;i<argc;i++)
	{
		...
		for(j=0;j<jj;j++)
		{
			if(argv[i][j]>='A'&&argv[i][j]<='Z') word[i-2]=word[i-2]*233+argv[i][j]-'A'+1;
	...
	for(i=0;i<n;i++)
	{
		...
		if(s[i]>='a'&&s[i]<='z') x=(x*p+s[i]-'a'+1)%mod;
		else if(x)
		{
			dic[++t]=x;
			x=0;
	...
	for(j=1;j<=m;j++) if(dnk[j]) idfk[j]=log(1.0*cnt/dnk[j])/log(10);
	//for(j=1;j<=m;j++) printf("%d %d %.2lf\n",j,dnk[j],idfk[j]);
	for(i=1;i<=cnt;i++)
		for(j=1;j<=m;j++)
