...
char map[127],s[40000010],end[1200010],id[70000][50];
int v[1200010][26],TNkd[20][70000],TNd[70000];
int i,j,k,p,n,cnt=1,N,sw,sa,DNk[20];
	...
	while(i<n)
	{
		...
		while(s[i]!=12)
		{
			...
			if(end[p])
			{
				...
				{
					int j=i-1;
					while((s[j]<123&&s[j]>96)||(s[j]<91&&s[j]>64)) j--;
					for(j=j+1;j<=i-1;j++) printf("%c",s[j]);
					printf(" %d\n",TNd[sa]);
				}
				++TNd[sa];
				++TNkd[end[p]][sa];
