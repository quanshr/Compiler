...
void InitVal()
{
	if(str!="{")
	{
		...
		Exp(x,0);
		int b=newtem();
		midcode[++mid_cnt]={"=const",b,(p++)*4};
		int c=newtem();
		miccode[++mid_cnt]={"+",c,nowvar,b};
		midcode[++mid_cnt]={"*=",c,x};
...
void LVal(int a,int _)      //_=1表示左值赋值，_=0表示取值 
{
	...
	else
	{
		...
		if(find_var(name).dim==dim)
		{
			if(_==0)
			{
				midcode[++mid_cnt]={"+",shftem,find_varcnt(name),shftem};
				midcode[++mid_cnt]={"=*",a,shftem};
				//midcode[++mid_cnt]={"[]",a,find_varcnt(name),shftem}; //取值
			}
			else
			{
				midcode[++mid_cnt]={"+",shftem,find_varcnt(name),shftem};
				midcode[++mid_cnt]={"*=",shftem,a};	
				//midcode[++mid_cnt]={"[]=",find_varcnt(name),shftem,a};    //赋值
			}	
		}
		else    //取址 
		{
		...
		if(a==0)
		{
			...
			if(var[funcnt].caninline)    // 可以进行inline内联优化 
			{
				...
				while(str!=")")		//形式传参 
				{
					...
					midcode[++mid_cnt]={"=",funcnt+paracnt,c};               //这块要改！！！若为数组，直接完全替换 
					paracnt++;
					if(str!=")") getsym(",");
				}
				getsym(")");
				...
				for(int i=var[funcnt].begin+paracnt;i<=var[funcnt].end;i++)
				{
					...
					if(midcode[mid_cnt].op=="label"||midcode[mid_cnt].op=="jump")
						midcode[mid_cnt].z=midcode[mid_cnt].z+label_cnt-var[funcnt].beginlabel;
					else if(midcode[mid_cnt].op=="beqz"||midcode[mid_cnt].op=="jumptofunc")
						midcode[mid_cnt].x=midcode[mid_cnt].x+label_cnt-var[funcnt].beginlabel;
				...
				label_cnt+=var[funcnt].endlabel-var[funcnt].beginlabel;
			}
			else	//不可内联 
			{
				getsym("(");
...
void FuncRParams(string tofuncname)
{
	...
	for(int i=1;i<=j;i++) midcode[++mid_cnt]={"inpara",a[i-1],i};    //还没有解决参数过多的情况 
}
// 传参  
...
//	for(int i=1;i<=mid_cnt;i++) if(midcode[i].op=="getint") printf("add $9 $10 $11\n");      //套取数据 
