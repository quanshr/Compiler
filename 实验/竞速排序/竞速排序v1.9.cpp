...
void fenpei(int x)
{
	...
	for(int i=basicblock[x].midcodes.size()-1;i>=0;i--)
	{
		MIDCODE code=basicblock[x].midcodes[i];
		string op=code.op;
		int z=code.z,x=code.x,y=code.y;
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			op=="%"||op=="!"||op=="!="||op=="=="||op==">="||op=="<="||
	...
	for(int i=0;i<basicblock[x].midcodes.size();i++)
	{
		MIDCODE code=basicblock[x].midcodes[i];
		string op=code.op;
		int z=code.z,x=code.x,y=code.y;
		if(finaluse[x]==i+1&&basicblock[x].out.count(x)==0) rgvar[varrg[x]]=0;
		if(finaluse[y]==i+1&&basicblock[y].out.count(y)==0) rgvar[varrg[y]]=0;
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			op=="%"||op=="!"||op=="!="||op=="=="||op==">="||op=="<="||
