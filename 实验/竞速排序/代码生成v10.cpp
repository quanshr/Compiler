...
void GVN(int p)
{
	...
	for(int j=0;j<basicblock[p].midcodes.size();j++)
	{
		...
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			...
			op==">"||op=="<"||op=="[]="||op=="[]")
			{
				...
				if(op=="[]="&&var[z].quanju!=1) z=varto[z];
			}
		else if(op=="inpara"||op=="inres"||op=="beqz"||op=="printint")
			if(var[z].quanju!=1) z=varto[z];
		...
		if(op=="="&&var[x].quanju!=1||
			op=="+"&&isconst[x]==1&&value[x]==0&&isconst[y]==0&&var[y].quanju!=1||
	...
	for(auto it:ini_varto) varto[it.first]=it.second;
}
