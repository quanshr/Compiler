...
void rename_getphi(int p)          //重命名并得到phi的右值 
{
	...
	for(int j=0;j<basicblock[p].midcodes.size();j++)
	{
		...
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			...
			op==">"||op=="<")
			{
				if(x!=0&&(var[x].nw!=0||var[x].type=="tem"))
				{
					if(st[x].empty())
					{
						st[x].push(++rename_cnt);
				...
				if(y!=0&&(var[y].nw!=0||var[y].type=="tem"))
				{
					if(st[y].empty())
					{
		...
		{
			if(z!=0&&(var[z].nw!=0||var[z].type=="tem"))
			{
				if(st[z].empty())
				{
...
void GVN(int p)
{
	...
	for(int j=0;j<basicblock[p].midcodes.size();j++)
	{
		...
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			...
			op==">"||op=="<")
			{
				basicblock[p].midcodes[j].x=varto[x];      //所有使用改为赋值的右值 
				basicblock[p].midcodes[j].y=varto[y];
			}
		else if(op=="inpara"||op=="inres"||op=="beqz"||op=="printint")
			basicblock[p].midcodes[j].z=varto[z];
		if(op=="=")
		{
			if(!change[z]) 	change[z]=varto[z];	
			varto[z]=basicblock[p].midcodes[j].x;          //更改以后的使用，复写传播，注意不能用x因为可能发生改变
		}
	}
...
int haveuse[100010];   //变量是否使用过
void dead_def_clear()
{
	for(int i=1;i<=mid_cnt;i++)    //主要操作于基本块内语句及addphi，它们参与生成目标代码 
	{
	...
	for(int i=1;i<=mid_cnt;i++)
	{
		...
		op=="=const")
		{
			if(haveuse[z]) midcode[++m]=midcode[i];           //如果使用过则加入这个定义 
		}
...
void fenkuai()   //分块 并优化 
{
	...
	{
		if(varinblock[i].size()<=1||var[i].nw==0||var[i].dim>0) continue;    //全局变量和数组不参与优化
		for(int j=1;j<=block_cnt;j++) vis[j]=0;
		for(int j=1;j<=block_cnt;j++)
		{
			/*
	...
	*/
	...
	for(auto mp:addphi)
	{
		...
		else   //只能为bnez 
		{
			...
			else
			{
				...
				final[++final_cnt]={"label",label};
				for(auto sentence:st)
				{
					final[++final_cnt]={"=",sentence.first,sentence.second};
				}
				final[++final_cnt]={"jump",nxlabel};
			}
		}
	}
	...
	//cout<<final_cnt<<endl;
	for(int i=1;i<=final_cnt;i++)
		midcode[++mid_cnt]=final[i];
	//dead_def_clear();
	//return;
	/*
	int c[100010];
