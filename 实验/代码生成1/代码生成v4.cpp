...
int getaddr(int cnt)
{
	if(var[cnt].type=="const") return base+var[cnt].base;
}
...
void LVal(int a,int _)      //_=1表示左值赋值，_=0表示取值 
{
	...
	else
	{
		...
		else
		{
			...
			int d=newtem();
			midcode[++mid_cnt]={"*",d,firsttem,c};
			int e=newtem();
			midcode[++mid_cnt]={"+",e,d,secondtem};
			shftem=e;
		}
		if(find_var(name).dim==dim)
		{
			if(_==0) midcode[++mid_cnt]={"[]",a,find_varcnt(name),shftem}; //取值
			else midcode[++mid_cnt]={"[]=",a,find_varcnt(name),shftem};    //赋值 如果是para可能有问题？？ 
		}	
		else    //取址 
		{
			int b=newtem();
...
void printmipscode()
{
	...
	for(int i=1;i<=mid_cnt;i++)
	{
		...
		{
			if(x==0) printf("addiu $9 $fp %d\n",(var[z].base+1)*4);  //x为所处层次 
			else printf("addiu $9 $sp %d\n",(var[z].base+1)*4);
			store(z,9);
		}
...
int main()
{
	...
	for(int i=1;i<=var_cnt;i++)
		cout<<i<<"   "<<var[i].type<<"  "<<var[i].base<<" "<<var[i].nw<<endl;*/
	printmipscode();
	return 0;
} 
