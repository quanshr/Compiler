...
int FuncDef()
{
	...
	var_tab[nw][name]=++var_cnt;
	var[var_cnt]={"func",dim,var_cnt,begin_label};    //存放返回值  seems to be useless  base存放var_cnt 
	Ident();
	getsym();  // (
	int begindata=data,beginmidcode=mid_cnt,return_label=++label;
	if(str!=")") FuncFParams();
	...
	for(int i=begindata;i<data;i++) midcode[++mid_cnt]={"pushdata",i};  //copy 
...
void Stmt(int begin_label,int end_label,int return_label)
{
	...
	else if(str=="return")
	{
		...
		if(str!=";")
		{
			int x=++tem_cnt;
			Exp(x,0);
			midcode[++mid_cnt]={"intmovetoreg",x,25};
		}
...
void LVal(int a,int _)
{
	...
	else if(dim==1)
	{
		...
		else
		{
			...
			midcode[++mid_cnt]={"loadword",a,0,b};
		}
	}
	else
	{
...
void FuncRParams(int beginvar_cnt)
{
	...
	if(var[beginvar_cnt].dim!=0)
	{
		...
		if(str=="[")
		{
			...
			midcode[++mid_cnt]={"inparadata",a0,find_var(name).base,a};
		}
		else
		{
			midcode[++mid_cnt]={"inparadata",a0,find_var(name).base,0};
		}
	}
	else
	{
		int a=++tem_cnt;
		Exp(a,0);
		midcode[++mid_cnt]={"inpara",a0,a};
	}
	while(str==",")
	{
		getsym(",");
		a0++;
		if(var[beginvar_cnt+a0-4].dim!=0)
		{
			string name=str;
			Ident();
			if(str=="[")
