...
int FuncDef()
{
	...
	var[var_cnt]={"func",dim,0,begin_label,nw,quanju};    //  base存放总data数量,var_cnt为返回值，后面为参数 
	Ident();
	getsym("(");
	if(str!=")") FuncFParams();
	getsym(")");
...
void GVN(int p)
{
	...
	for(int j=0;j<basicblock[p].midcodes.size();j++)
	{
		...
		if(op=="="||op=="+"||op=="-"||op=="*"||op=="/"||
			...
			op==">"||op=="<"||op=="[]="||op=="[]")
			{
				if(op!="[]"&&var[x].quanju!=1) basicblock[p].midcodes[j].x=varto[x];      //所有使用改为赋值的右值 
				if(var[y].quanju!=1) basicblock[p].midcodes[j].y=varto[y];
				//if(op=="[]="&&var[z].quanju!=1) basicblock[p].midcodes[j].z=varto[z];
			}
		else if(op=="inpara"||op=="inres"||op=="beqz"||op=="printint")
			if(var[z].quanju!=1) basicblock[p].midcodes[j].z=varto[z];
		if(op=="="&&var[z].quanju!=1)
		{
