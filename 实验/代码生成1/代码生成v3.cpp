...
int FuncDef()
{
	...
	for(int i=data-4;i>=begindata;i--) midcode[++mid_cnt]={"popdata",i};
                                        ...
                                        //若为参数，base=0,结果全部存于tem 
{
	...
	{
		base=0;
		midcode[++mid_cnt]={"lw",13,find_var(name).base,30};
...
void FuncRPara(int nowvar_cnt,int a)
{
	if(var[nowvar_cnt].dim!=0)
	{
		...
		if(find_var(name).type=="para")  //若为从参数1传参数2，参数1维度一定大于等于2,因此参数1一定为地址 
		{
			if(dim==0)
			{
				midcode[++mid_cnt]={"lw",9,find_var(name).base,30};
		...
		else
		{
			...
			else
			{
				...
				midcode[++mid_cnt]={"addi",9,9,find_var(name).base};
				midcode[++mid_cnt]={"add",9,9,30};
				midcode[++mid_cnt]={"regtoint",9,a};
			}
		}
