...
void Stmt(int begin_label,int end_label,int return_label)
{
	...
	else if(str=="return")
	{
		...
		if(str!=";")
		{
			int x=++tem_cnt;
			Exp(x,0);
			midcode[++mid_cnt]={"inttoreg",x,25};         //25号寄存器存储函数返回值 
		}
	...
	else
	{
		...
		if(nextsym(j)=="=")
		{
			...
			else
			{
				...
				Exp(c,0);
				getsym(";");
				midcode[++mid_cnt]={"storeword",c,b,a};
			}
		}
		else
		{
...
void FuncRPara(int nowvar_cnt,int reg,int b);
void FuncRParams(int beginvar_cnt)
{
	int reg=4;
	int a[5],j=0;
	a[j++]=++tem_cnt;
	FuncRPara(beginvar_cnt,reg,a[j]);
	while(str==",")
	{
		getsym(",");
		reg++;
		a[j++]=++tem_cnt;
		FuncRPara(beginvar_cnt+reg-4,reg,a[j]);
	}
	for(int i=0;i<j;i++) midcode[++mid_cnt]={"inttoreg",a[i],i+4};
}
// 传参 
/*
void FuncRPara(int nowvar_cnt,int reg)
{
	if(var[nowvar_cnt].dim!=0)
	{
		string name=str;
		Ident();
		if(find_var(name).type=="para"&&find_var(name).dim!=1)
		{
			//cout<<"WWWWWDADSA";
			if(str=="[")
			{
				...
				midcode[++mid_cnt]={"*const",a,a,find_var(name).second*4};
				midcode[++mid_cnt]={"load",a,9};
				midcode[++mid_cnt]={"lw",reg,find_var(name).base*4,30};
				midcode[++mid_cnt]={"add",reg,reg,9};
			}
			else
			{
				midcode[++mid_cnt]={"lw",reg,find_var(name).base*4,30};
		...
		{	
			if(str=="[")
			{
				...
				Exp(a,0);
				getsym("]");
				//cout<<reg<<"   "<<find_var(name).second<<endl;
				midcode[++mid_cnt]={"*const",a,a,find_var(name).second};
				midcode[++mid_cnt]={"inparaaddr",reg,find_var(name).base,a};
			}
			else midcode[++mid_cnt]={"inparaaddr",reg,find_var(name).base,0};
		}
	}
	else
	{
...
void FuncRPara(int nowvar_cnt,int reg,int b)
{
	...
	midcode[++mid_cnt]={"regtoint",reg,b};
}
//UnaryOp → '+' | '-' | '!'
int UnaryOp()
{
...
int tem_data[1000010],strdata=10000;
